## BackTest Code
    ...
    Cases1 = 0;//Optimize("Cases1",0,0,3,1);
    ...
    buycount = SumSince(Day()!=Ref(Day(),-1),Buy);
    finalcount = iif(buycount==1 AND Ref(buycount,-1)==0,1,0);
    Buyday = ValueWhen(Buy==1,DateNum());
    WeeklyExpiryLastbar = IIf( DateNum()==weeklyexpirydate AND Day() != Ref(Day(),1),1,0);

    profitpercent = (C/ValueWhen(finalcount,Close)-1)*100;
    pt = Optimize("pt",1000,100,1000,100);
    profittarget = IIf(profitpercent > pt ,1,0);

    Sell = IIf(Buyday==WeeklyExpiryDate AND profittarget==1,1,IIf(Buyday==WeeklyExpiryDate,TimeNum()>=ExitTime OR DateNum()!=Ref(DateNum(),1),TimeNum()<=NextDayExitTime)) OR exitlastbar;
    ...
    Filter =1;
    AddColumn(Buy,"Buy");
    AddColumn(buycount,"buycount");

    AddColumn(finalcount,"finalcount");
    AddColumn(profitpercent,"profitpercent");
